...
function Forms() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/typesRequest"
        );
  ...
  const handleLinkClick = async (popId) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/typesRequest/${popId}`
      );
  ...
  return (
    <div className="formsrapper">
      ...
      {links.map((link, index) => (
        ...
        >
          <div className="linktdem">{link.title}</div>
        </div>
      ))}
        ...
        <div className="popModal">
          <div className="popContent">
                ...
                <li key={index}>
                  ...
                  <div className="pdf-field">
                    <label className="file-label">
                      ...
                      <span className="file-text">{fileNames[index]}</span>{" "}
                      {/* Utilisez l'index pour le texte du fichier */}
                    </label>
                  </div>
                </li>
              ))}
            ...
            <div className="buttonsWrapper">
              <button className="cancelButton" onClick={closePop}>
                Cancel
              </button>
              <button className="validateButton" onClick={handleSubmit}>
                Validate
              </button>
            </div>
          </div>
